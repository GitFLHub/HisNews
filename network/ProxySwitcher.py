...
class ProxySwitcher:
    ...
    def load_clash_host_config(self):
        # 指定宿主机Clash配置文件的路径
        clash_config_path = '/mnt/c/Users/dell/.config/clash/config.yaml'
        try:
    ...
    def switch_proxy(self, proxies_info):
        ...
        while selector_time < 8:
            ...
            if response.status_code == 200:
                ...
                if delay < 800:
                    print(f"延迟 {delay} ms, 可用。")
                    break
                # 等待 1 秒
                time.sleep(1)
            else:
                print(f"获取节点 {new_proxy} 延迟失败: {response.status_code}")
                selector_time += 1
    ...
    # 其余方法（switch_proxy, switch_proxy_auto）保持不变
